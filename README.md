...
### 最終更新 2021-09-29
- 這是最後一次更新，將原本的跟隨游標改成相容性較好的跟隨螢幕。
- 之後重心放在 V2 版開發的版本，V1 版將不再更新，只做為開發 V2 版本的參考。
- AutoHotkey V2 語法比 V1 方便很多且較一致性，可讀性也較好，許多 AHK 第三方開發人員也都陸續轉移到 AHK v2 了，因此 V2 將會是趨勢。
...
另外也要感謝蝦友 fairry 提供擴展區字集與韓語表格，以及提供線上轉檔網站，讓我方便做出系統列圖示。
- 修正: 修正內部全形符號的送字模式判斷錯誤
...
- `Table`資料夾內放的是表格檔，`Icon`資料夾放的是圖示檔，`Config`資料夾放的是其它設定檔與字型檔(ttf 或 ttc)
- `Config` 中有一個程式 UniteTTC.exe 是拆 TTC 字型的工具，遇到不支援的 TTC 可以使用這個工具拆出 TTF 使用。
- 程式執行時，會載入 `Config\pos.ini` 儲存的座標位置，輸入介面有變更位置時會更新此檔案。刪除此檔案，執行時輸入介面會以預設值顯示在螢幕左下角。
...
## Config\Clipboard.txt 與 Config\Fixed.txt
- 檔案內容為視窗的 Class Name 清單，或是視窗標題，程式執行時會讀取該檔。
- `Clipboard.txt`→若是焦點視窗有符合 Class Name 或視窗標題，就會自動以剪貼簿送字。
- `Fixed.txt`→若是焦點視窗有符合 Class Name 或視窗標題，會強制停用游標跟隨功能。  
