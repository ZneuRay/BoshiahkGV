...
目前已測試的情況:  
1.若是螢幕有設定不同的縮放值，有些程式無法抓到正確的輸入游標位置
2.Chrome 在延伸螢幕會無法取得輸入游標位置。
3.Office、Line 沒有問題。
4.原生 GUI 沒有問題。
5.部分程式如 Notepad++ 會因為雙螢幕的縮放比例不同，造成程式抓游標位置不正確，修正方式就是把兩台螢幕的 DPI 設定調成一樣。
...
## Tbl\fixed.txt
- 檔案內容為視窗的 Class Name 清單，程式執行時會讀取該檔。
- 開啟輸入介面時，若是焦點視窗 Class Name 有符合的，會強制停用游標跟隨功能。
- 可以用 Ctrl-Alt-Shift-C 查詢焦點視窗的 Class Name。
